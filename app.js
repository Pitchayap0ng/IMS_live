...
// เปิด Offline Persistence: หัวใจหลักที่ทำให้ข้อมูลออฟไลน์ไม่หายและโหลดเร็ว
...
db.ref('money_flow').on('value', snapshot => {
    ...
    // อัปเดตหน้าจอที่กำลังแสดงอยู่ปัจจุบัน
...
function saveTransaction() {
    ...
    Swal.fire({ title: 'บันทึกสำเร็จ', icon: 'success', timer: 800, showConfirmButton: false });
}
...
function renderDaily() {
    ...
    if (listContainer) {
        listContainer.innerHTML = todayTrans.map(t => `
...
function renderStats() {
    ...
    if (summaryDiv) {
        summaryDiv.innerHTML = `
            <div class="p-4 rounded-3xl bg-emerald-50 dark:bg-emerald-900/10">
            ...
            <div class="p-4 rounded-3xl bg-rose-50 dark:bg-rose-900/10">
